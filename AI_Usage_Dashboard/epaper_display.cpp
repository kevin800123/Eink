...
bool EpaperDisplay::initializeFullRefresh() {
  ...
  if (!loadFullRefreshWaveform()) {
    setError("e-paper BUSY timeout while loading LUT data");
    return false;
  }
  if (!waitUntilIdle()) {
...
bool EpaperDisplay::loadFullRefreshWaveform() {
  sendCommand(0x32);
  sendData(kFullRefreshWaveform, 153);
  // Waveshare's EPD_SetLut waits here, between the LUT block and the drive
  // voltage registers. port_display.cpp:143-161.
  if (!waitUntilIdle()) {
    return false;
  }
